...
class Command(BaseCommand):
    ...
    def verify_neo4j_connection(self, debug):
        ...
        if debug:
            self.stdout.write(
                self.style.SUCCESS(Colors.amber(f"NEO4J_USERNAME: {NEO4J_USERNAME}"))
            )
            self.stdout.write(
                self.style.SUCCESS(Colors.amber(f"NEO4J_PASSWORD: {NEO4J_PASSWORD}"))
            )
            self.stdout.write(
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    Colors.amber(f"NEOMODEL_NEO4J_BOLT_URL: {NEOMODEL_NEO4J_BOLT_URL}")
                )
            )
    ...
    def verify_redis_connection(self, debug):
        ...
        if debug:
            self.stdout.write(
                self.style.SUCCESS(Colors.teal(f"REDIS_PASSWORD: {REDIS_PASSWORD}"))
            )
            self.stdout.write(
    ...
    def verify_postgres_connection(self, debug):
        ...
        if debug:
            self.stdout.write(
                self.style.SUCCESS(Colors.orange(f"POSTGRES_DB: {POSTGRES_DB}"))
            )
            self.stdout.write(
                self.style.SUCCESS(Colors.orange(f"POSTGRES_USER: {POSTGRES_USER}"))
            )
            self.stdout.write(
                self.style.SUCCESS(
            ...
            self.stdout.write(
                self.style.SUCCESS(Colors.orange(f"POSTGRES_HOST: {POSTGRES_HOST}"))
            )
            self.stdout.write(
                self.style.SUCCESS(Colors.orange(f"POSTGRES_PORT: {POSTGRES_PORT}"))
            )
